...
from app.services.mongo_service import create_job, get_job, list_jobs, update_job, mark_job_posted, toggle_hiring_active, log_activity
...
## How to Apply
Send your CV to: {req.hr_email or 'our recruitment email'}
Subject: "Application for {req.title} - [Your Name]"
""".strip()
                job_data["short_description"] = (
                    ...
                    f"📩 Apply by sending your CV to: {req.hr_email or 'our recruitment email'}\n"
                    f"Subject: 'Application for {req.title} - [Your Name]'\n\n"
...
async def delete_job_endpoint(job_id: str):
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    await job.delete()
    await log_activity("jd", f"Job deleted: <strong>{job.title}</strong>", color="#E05555", job_id=job_id)
    return {"deleted": True}


@router.post("/{job_id}/toggle-hiring")
async def toggle_hiring(job_id: str):
    """
    Enable or disable active hiring for a job.
    When disabled, any incoming CV emails for this job receive a
    professional 'position filled' response automatically.
    """
    job = await get_job(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    updated = await toggle_hiring_active(job_id, not job.hiring_active)
