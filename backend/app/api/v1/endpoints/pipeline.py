                    ...
                    if send_emails and candidate_email:
                        if doc.final_decision == "MATCH":
                            if settings.AUTO_SEND_ASSESSMENT:
                                logger.info(f"[Pipeline {run_id}] Auto-sending assessment to {candidate_email}")
                                try:
                                    ...
                                    sent = smtp.send_assessment_invitation(
                                        to=candidate_email,
                                    ...
                                    if sent:
                                        ...
                                        await log_activity(
                                            "email",
                                            f"✅ Assessment invitation sent to <strong>{candidate_name}</strong> ({score}%)",
                                            color="#9B59B6",
                                            candidate_id=str(doc.id),
                                            job_id=job_id,
                                        )
                                except Exception as e:
                                    logger.error(f"[Pipeline {run_id}] Failed to create assessment: {e}")
                            else:
