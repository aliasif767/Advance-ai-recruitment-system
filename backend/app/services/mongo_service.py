...
async def toggle_hiring_active(job_id: str, active: bool) -> Optional[JobDocument]:
    """Enable or disable active hiring for a job."""
    job = await get_job(job_id)
    if not job:
        return None
    await job.set({"hiring_active": active, "updated_at": datetime.utcnow()})
    status_label = "🟢 Hiring activated" if active else "🔴 Hiring paused"
    await log_activity(
        "jd",
        f"{status_label} for <strong>{job.title}</strong>",
        color="#3DB87A" if active else "#E05555",
        job_id=job_id,
    )
...
async def save_candidate(report: dict, job_id: str | None = None) -> CandidateDocument:
    ...
    doc = CandidateDocument(
        ...
        job_id=job_id,
        job_title=job_title,
        applied_role=report.get("applied_role", ""),
...
async def get_global_stats() -> dict:
    ...
    # Assessment stats
    try:
        from app.db.interview_models import AssessmentDocument
        total_assessments = await AssessmentDocument.count()
        completed_assessments = await AssessmentDocument.find({"status": {"$in": ["submitted", "evaluated"]}}).count()
        in_progress_assessments = await AssessmentDocument.find({"status": "active"}).count()
        evaluated_docs = await AssessmentDocument.find({"status": "evaluated"}).to_list()
        scores = [a.final_composite_score for a in evaluated_docs if a.final_composite_score > 0]
        avg_assessment_score = round(sum(scores) / len(scores), 1) if scores else 0
        assessments_sent = await AssessmentDocument.find({"status": {"$ne": "pending"}}).count()
    except Exception:
        total_assessments = completed_assessments = in_progress_assessments = avg_assessment_score = assessments_sent = 0
    ...
    return {
        ...
        # Assessment stats for dashboard widget
