...
class IMAPClient:
    ...
    def fetch_applications(self, mailbox: str = "INBOX") -> List[IngestedApplication]:
        """
        Connects to IMAP, finds UNSEEN messages in the specified mailbox,
        ...
        try:
            ...
            if status != "OK":
                logger.error(f"Mailbox {mailbox} not found.")
                mail.logout()
                return results

            # Search ONLY unread emails in the Primary category from the last 2 days
            status, messages = mail.search(None, 'X-GM-RAW', '"category:primary is:unread newer_than:2d"')
            if status != "OK" or not messages[0]:
                logger.info(f"No unread emails found in {mailbox}.")
                mail.logout()
                return results
